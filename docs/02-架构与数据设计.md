...
- 一套 Expo Web 客户端按角色显示六种界面，并适配桌面、平板和手机浏览器。
- 普通读取可以通过 Supabase SDK 和 RLS 完成。
- 分值、币账、罚款、成绩发布、撤销等关键写操作必须经过数据库函数或 Edge Function。
- DeepSeek 只返回白名单意图结构；数据查询、权限判断和写操作始终由海豚云服务端完成。
...
| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `grade_report_sheets` | `teacher_id`, `class_id`, `subject`, `title`, `source`, `status`, `published_at` | 一次整体发布的成绩单 |
| `grade_report_columns` | `sheet_id`, `column_key`, `name`, `position`, `max_score?` | 一个或多个成绩项 |
| `grade_report_rows` | `sheet_id`, `student_id` | 成绩单中的学生行 |
| `grade_report_values` | `row_id`, `column_id`, `score`, `comment` | 学生在每个成绩项下的当前值 |
| `grade_report_value_revisions` | `value_id`, `old_score`, `new_score`, `reason`, `actor_id` | 发布后不可变修订历史 |

一张成绩单至少包含一个成绩项和一个学生行，所有行必须完整覆盖全部成绩项。`score` 使用 `numeric(7,2)`，不得与学生分的整数分值字段共用。家庭读取按绑定学生过滤整行，不返回同班其他学生。管理端默认只读成绩单和成绩项元数据，不读取学生行或成绩值。

`assessments`、`grade_records` 和 `grade_revisions` 是已经发布的单成绩字段兼容契约，迁移期间继续保留；新功能统一使用 `grade_report_*` 模型，不改写旧迁移或破坏旧 API。
...
| `ai_sessions` | `user_id`, `coze_conversation_ref`, `status` | AI 会话映射；字段名为早期兼容名称，不保存模型密钥 |

...
create table public.grade_report_columns (
  ...
  max_score numeric(7, 2) check (max_score between 0 and 99999.99),
  created_at timestamptz not null default now(),
...
create table public.grade_report_values (
  ...
  score numeric(7, 2) not null check (score between 0 and 99999.99),
...
create or replace function public.is_valid_grade_report_decimal(value jsonb)
returns boolean
language plpgsql
immutable
security definer
...
begin
  ...
  begin
    parsed_value := (value #>> '{}')::numeric;
  exception when others then
    return false;
  end;
  return parsed_value between 0 and 99999.99
    and scale(parsed_value) <= 2;
end;
$$;

create or replace function public.is_valid_grade_report_position(value jsonb)
returns boolean
language plpgsql
immutable
security definer
...
begin
  ...
  return parsed_value between 0 and 49 and scale(parsed_value) = 0;
end;
$$;
...
as $$
  select exists (
    ...
    where report_row.id = target_row_id
      and (
        public.is_grade_report_sheet_owner(report_sheet.id)
        or (
...
as $$
  select exists (
    ...
    join public.grade_report_sheets as report_sheet on report_sheet.id = report_row.sheet_id
    where report_value.id = target_value_id
      and public.is_grade_report_sheet_owner(report_sheet.id)
  );
$$;
...
begin
  ...
  if coalesce(char_length(btrim(sheet_title)) between 1 and 120, false) is false
    or coalesce(char_length(btrim(sheet_subject)) between 1 and 60, false) is false
    or coalesce(sheet_source in ('grid', 'csv', 'xlsx'), false) is false then
    raise exception 'VALIDATION_ERROR';
  end if;
  if coalesce(jsonb_typeof(normalized_columns), '') <> 'array'
    or coalesce(jsonb_typeof(normalized_rows), '') <> 'array' then
    raise exception 'VALIDATION_ERROR';
  end if;
  if jsonb_array_length(normalized_columns) not between 1 and 50
  ...
  if exists (
    ...
    where jsonb_typeof(item.value) <> 'object'
      ...
      or not public.is_valid_grade_report_position(item.value -> 'position')
      or (
        item.value ? 'max_score'
        and jsonb_typeof(item.value -> 'max_score') <> 'null'
        and not public.is_valid_grade_report_decimal(item.value -> 'max_score')
      )
  ) then
    raise exception 'VALIDATION_ERROR';
  end if;
  ...
  if exists (
    ...
    where jsonb_typeof(report_row.value) <> 'object'
      ...
      or coalesce(jsonb_typeof(report_row.value -> 'values'), '') <> 'array'
  ) then
    raise exception 'VALIDATION_ERROR';
  end if;
  ...
  if exists (
    select 1
    from jsonb_array_elements(normalized_rows) as report_row(value)
    where jsonb_array_length(report_row.value -> 'values') <> column_count
  ) then
    raise exception 'VALIDATION_ERROR';
  end if;
  ...
  if exists (
    ...
    where jsonb_typeof(report_value.value) <> 'object'
      ...
      or not public.is_valid_grade_report_decimal(report_value.value -> 'score')
      or coalesce(jsonb_typeof(report_value.value -> 'comment'), '') <> 'string'
...
create or replace function public.list_grade_report_sheets_for_class(
  target_class_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  select coalesce(
    jsonb_agg(
      public.grade_report_sheet_payload(report_sheet.id)
      order by report_sheet.updated_at desc, report_sheet.id
    ),
    '[]'::jsonb
  )
  into result
  from public.grade_report_sheets as report_sheet
  where report_sheet.class_id = target_class_id
...
create or replace function public.get_grade_report_sheet(target_sheet_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return public.grade_report_sheet_payload(target_sheet_id);
end;
$$;
...
begin
  ...
  if revised_score is null
    ...
    or revised_score not between 0 and 99999.99
    or scale(revised_score) > 2
  ...
  where report_value.id = target_value_id
    ...
    and report_sheet.teacher_id = actor_id
    and report_sheet.status = 'published'
    and public.is_teacher_assigned_to_class(actor_id, report_sheet.class_id)
...
revoke all on function public.is_valid_grade_report_decimal(jsonb) from public;
revoke all on function public.is_valid_grade_report_position(jsonb) from public;
...
revoke all on function public.list_grade_report_sheets_for_class(uuid) from public;
revoke all on function public.get_grade_report_sheet(uuid) from public;
...
grant execute on function public.list_grade_report_sheets_for_class(uuid) to authenticated;
grant execute on function public.get_grade_report_sheet(uuid) to authenticated;
